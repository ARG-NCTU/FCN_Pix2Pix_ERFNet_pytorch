...
void mask_to_point::callback(const subt_msgs::arti_input msg){
	...
	pc_pub_mask.publish(object_cloud_msg);
	return;
}
mask_to_point::mask_to_point(){
	NodeHandle nh;

	pc_pub_mask = nh.advertise<sensor_msgs::PointCloud2> ("/mask_to_point_result", 10);
	FCN_Pix2Pix_result = nh.subscribe<subt_msgs::arti_input>("/mask_to_point", 1, &mask_to_point::callback,this); 
